...
f32 fnGgxDistribution(f32 dotNorH, f32 a2) {
	f32 d = dotNorH * dotNorH * (a2 - 1.0) + 1.0;
	d = max(d, 1e-5);
	return a2 / (PI * d * d);
}

// Height-correlated Smith visibility;
// includes the 1/(4 dotNorWi dotNorWo) denominator.
...
// Build orthonormal basis from N and reorient
f32v3 fnReorientHemisphere(f32v3 v, f32v3 N) {
	const f32v3 up = (
		abs(N.z) < 0.999 ? f32v3(0.0, 0.0, 1.0) : f32v3(1.0, 0.0, 0.0)
	);
